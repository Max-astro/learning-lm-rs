...
fn mlp(
    ...
    let mut act = gate.slice(0, gate.shape());
    OP::swiglu(&mut act, up);
    // OP::element_wise_product(&mut act, gate);
    // OP::element_wise_product(&mut act, up);
    let mut output = Tensor::<f32>::default(&vec![act.shape()[0], w_down.shape()[0]]);
    OP::matmul_transb(&mut output, 0., &act, w_down, 1.);
    OP::element_wise_op(residual, &output, |x, y| x + y);
}
